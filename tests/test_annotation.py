...
class TestAnnotationAPI:
    ...
    def test_annotate_endpoint_success(self, mock_generate, override_deps):
        ...
        assert (
            'attachment; filename="draft_annotated.md"' in response.headers["Content-Disposition"]
        )
...
def test_generate_draft_annotations_uses_comparison_rows(tmp_path, override_deps):
    ...
    draft_path.write_text(
        "The model always works on every input. The second sentence stays unchanged.",
        encoding="utf-8",
    )

    db = SessionLocal()
    session = AnalysisSession(
        id="session-1", user_id="test-user-123", name="session", status="completed"
    )
    draft = Document(
        id="draft-doc-1",
        user_id="test-user-123",
    ...
    with patch(
        "app.services.draft_annotation_service.generate_alternative_rephrasing",
