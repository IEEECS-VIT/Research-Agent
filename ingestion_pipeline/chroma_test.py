...
from ingestion_pipeline.chroma_store import CHROMA_DATA_DIR, store_document_in_chroma
...
async def test_vector_store():
    ...
    try:
        ...
        if total_items > 0:
            ...
            logger.warning(
                "[WARNING] ChromaDB collection opened successfully but no items were tracked."
            )

    except Exception as e:
        logger.error(
            "[FAIL] Error attempting to read or readback verification from ChromaDB: %s", e
        )
