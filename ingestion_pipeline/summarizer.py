...
async def summarise_image(image, image_index: int) -> str:
    ...
    logger.warning("Continuing without generated summary for image %d.", image_index)
...
async def batch_summarise_sections(sections: list[dict]) -> list[SectionSummary]:
    ...
    for sec in sections:
        ...
        payload_data.append({"section_name": sec["section_name"], "text": text_snippet})
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                "Executing API Call (Attempt %d/%d) using %s...",
                attempt + 1,
                max_retries,
                MODEL_NAME,
            )
            ...
            for sec in sections:
                ...
                final_results.append(
                    SectionSummary(
                        section_name=name, raw_text=sec["raw_text"], summary=summary_text
                    )
                )

            logger.info(
                "Single-Shot Generation Complete! Extracted %d summaries.", len(final_results)
            )
            ...
            if "quota" in err_str or "429" in err_str or "exhausted" in err_str:
                if attempt < max_retries - 1:
                    logger.warning(
                        "Rate limit hit. Google API is in timeout. Waiting %d seconds before retry...",
                        delay,
                    )
                    await asyncio.sleep(delay)
                    delay += 15
                    ...
                    raise RuntimeError(
                        "Max retries exceeded due to rate limits. Please check your API quota."
                    ) from e
            else:
...
async def run_pipeline(file_path: str) -> tuple[list[SectionSummary], str | None]:
    try:
        ...
        doi = extract_doi(
            parsed_content.full_text[:3000]
        )  # scan the leading text where DOI metadata usually appears
